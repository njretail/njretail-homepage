    ...
    <>
      ...
      <main className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <section className="text-center">
          <p className="mb-4 text-xs font-semibold uppercase tracking-[0.22em] text-[#3B82F6]">Case</p>
          ...
          <div className="mt-10 grid gap-8 md:grid-cols-2 xl:grid-cols-3">
            {visibleCases.map((item) => (
              <article key={item.title} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                <div className="p-6">
                  <div className="inline-flex rounded-full bg-sky-50 px-2.5 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-[#3B82F6]">
                    {item.category}
                  </div>
                  ...
                  <Link href="/contact" className="mt-5 inline-flex text-sm font-semibold text-[#3B82F6] hover:text-[#1d4ed8]">
                    상담 신청 →
                  </Link>
                </div>
              </article>
