        ...
        {steps.map((s, i) => (
          <div key={s.n} className="flex items-center gap-2">
            <div className="flex items-baseline gap-2">
              <span className="font-mono text-sm font-bold text-black">{s.n}</span>
              <span className="text-sm font-semibold text-black">{s.label}</span>
              <span className="text-sm text-black/40">{s.sub}</span>
            </div>
            {i < steps.length - 1 && <span className="ml-2 text-black/20 lg:ml-4">&#8594;</span>}
          </div>
        ))}
      </div>
    </div>
            ...
            <section className="border-t border-black/10 bg-[#F5F3EF]">
              <div className="mx-auto grid max-w-7xl lg:grid-cols-[2fr_3fr]">
                <div className={`flex flex-col justify-center px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24 ${isEven ? "lg:order-1" : "lg:order-2"}`}>
                  ...
                  {p.icons && (
                    <div className="relative mt-10 flex h-40 w-full max-w-sm items-center justify-center border border-black/10 bg-white/60">
                      <span className="absolute h-24 w-24 rounded-full border border-dashed border-black/15" />
                      <span className="absolute h-14 w-14 rounded-full border border-dashed border-black/20" />
                      <span className="relative flex h-3 w-3 items-center justify-center rounded-full bg-black">
                  ...
                  {p.icons && (
                    <div className="mt-10 grid max-w-md grid-cols-3 gap-4 border-t border-black/10 pt-6">
                      {p.icons.map((item) => (
                        <div key={item.label}>
                          <div className="flex h-8 w-8 items-center justify-center border border-black/15 text-sm text-black/60">{item.icon}</div>
    ...
    <section className="border-t border-black/10">
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[2fr_3fr]">
        <div className="bg-black px-4 py-16 text-white sm:px-6 sm:py-20 lg:px-8 lg:py-24">
          <p className="font-mono text-xs uppercase tracking-[0.3em] text-white/40">Startup Cost</p>
        ...
        <div className="relative bg-[#F5F3EF] px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
          <div className="max-w-xl border-t border-black/15 font-mono text-sm">
            ...
            <div className="flex items-baseline justify-between py-4 text-base font-bold text-black">
          ...
          <div className="pointer-events-none absolute inset-y-0 right-8 hidden flex-col items-center justify-center gap-4 lg:flex">
            ...
            <div className="rotate-180 font-mono text-[10px] uppercase tracking-[0.3em] text-black/40" style={{ writingMode: "vertical-rl" }}>
              NJ RETAIL / STORE 01
            </div>
          </div>
        </div>
